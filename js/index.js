...
$(function ()
{
	...
	$("#slider").draggable({ axis: "x" ,start: function() {
      },
      drag: function() {
      },
      stop: function() {
					...
					if(AC_DOWNLOAD & mask)
					{
						temp.download_file = 
						{
							...
							"action"			: function (data)
							{
								...
								if(confirm("Download the image: " + name + " ?"))
								{
									var url = "lib/download.php?id=" + encodeURIComponent(obj.id);
									var new_win = window.open(url, "Download", "toolbar=yes, scrollbars=yes, resizable=yes, top=400, left=400, width=600, height=400");
					...
					if(AC_UPLOAD & mask)
					{
						temp.upload_images =
						{
							...
							"action"			: function (data)
							{
								var inst = $.jstree.reference(data.reference),
									obj = inst.get_node(data.reference);
								g_url = 'plugin_manager.php?folder=' + obj.id;
						...
						temp.set_mods =
						{
							...
							"action"			: function (data)
							{
								var inst = $.jstree.reference(data.reference),
									obj = inst.get_node(data.reference);
								var mod = 0;
								if(confirm("Set Filename Parsing for: " + obj.id + " current mods: " + obj.original.mods))
								{
									mod = 1;
								}
	...
	.on('create_node.jstree', function (e, data) {
		$.get('lib/MenuOps.php?operation=create_node', { 'type' : data.node.type, 'id' : data.node.parent, 'text' : data.node.text })
			.done(function (d) {
